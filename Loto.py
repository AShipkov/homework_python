...
class barrel:
    ...
    def f(self):                                            # Достаем по 1
        list = [x for x in range(1, 91)]
        random.shuffle(list)
        for i, y in enumerate(list):
            print('{:*^28}'.format('*'))
            print('Новый бочонок: {} (всего осталось {})'.format(y, self.amount - (i + 1)))
            yield y
...
class Loto:
    ...
    def search(self, card_player, num_cask):                  # Поиск номера на карточке и определение победителя
...
def main():
    ...
    while True:
        ...
        inp_user = input('Зачеркнуть число? Введите y или n: ')
        if inp_user == 'y':
